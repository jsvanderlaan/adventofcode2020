...
(string adj, string col) yours = ("shiny", "gold");
(string adj, string col) end = (null, null);

Func<string, (string adj, string col)> splitBag = (string bag) => (bag.Split(" ")[0], bag.Split(" ")[1]);
Func<string, (int num, (string adj, string col) bag)> splitInner = (string inner) => (int.Parse(inner.Split(" ")[0]), (inner.Split(" ")[1], inner.Split(" ")[2]));

var edges = data
    .Replace("\r", string.Empty)
    .Replace("\n", string.Empty)
    .Split(".").Where(rule => !string.IsNullOrWhiteSpace(rule))
    .SelectMany(rule =>
    {
        var bags = rule.Split(" contain ");
        var outer = splitBag(bags[0].Replace(" bags", string.Empty));
        var allInner = bags[1].Split(", ").Select(inn => inn == "no other bags"
            ? (0, end)
            : splitInner(inn.Replace(" bags", string.Empty)));

        return allInner.Select(((int num, (string adj, string col) bag) inner) => (outer, inner));
    });

Func<IEnumerable<(string adj, string col)>, IEnumerable<(string adj, string col)>> getOuter = null;

getOuter = (bags) => bags.Count() == 0
    ? new (string adj, string col)[] { }
    : bags.Concat(getOuter(bags
        .SelectMany(bag => edges.Where(edge => edge.inner.bag == bag).Select(edge => edge.outer))));

var ans = getOuter(new[] { yours }).Where(bag => bag != yours).Distinct().Count();
Console.WriteLine(ans);

Func<(string adj, string col), int> countInner = null;

countInner = (bag) => edges
    .Where(edge => edge.outer == bag)
    .Select(edge => edge.inner)
    .Aggregate(0, (count, next) => count + (next.bag == end ? 0 : next.num + (next.num * countInner(next.bag))));

var ans2 = countInner(yours);
