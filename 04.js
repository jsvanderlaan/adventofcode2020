...
const ans = data
  .replace(/ \n/g, " ")
  .split("  ")
  .map((passport) => passport.match(/(\w*):/g))
  .filter((items) => requiredFields.every((field) => items.some((item) => item === `${field}:`))).length;
...
const validField = (field) => {
  switch (field[0]) {
    case "byr":
      return field[1].match(/^\d{4}$/) && field[1] <= 2002 && field[1] >= 1920;
    case "iyr":
      return field[1].match(/^\d{4}$/) && field[1] <= 2020 && field[1] >= 2010;
    case "eyr":
      return field[1].match(/^\d{4}$/) && field[1] <= 2030 && field[1] >= 2020;
    case "hgt":
      const cm = field[1].match(/^(\d{3})cm$/);
      const ins = field[1].match(/^(\d{2})in$/);
      if (cm) return cm[1] <= 193 && cm[1] >= 150;
      if (ins) return ins[1] <= 76 && ins[1] >= 59;
      return false;
    case "hcl":
      return field[1].match(/^#[0-9a-f]{6}$/);
    case "ecl":
      return field[1].match(/^amb|blu|brn|gry|grn|hzl|oth$/);
    case "pid":
      return field[1].match(/^\d{9}$/);
    default:
      return true;
  }
};
...
const ans2 = data
  .replace(/ \n/g, " ")
  .split("  ")
  .map((passport) => passport.split(" ").map((x) => x.split(":")))
  .filter((items) => requiredFields.every((field) => items.some((item) => item[0] === `${field}`)) && items.every(validField)).length;
